...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return generateIpoPageMetadata('allotmentStatus', basicData, detailData);
}
...
export default async function AllotmentStatusPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  ...
  return (
    <main className="container py-10">
      <h1 className="text-3xl font-bold">Allotment Checker</h1>
