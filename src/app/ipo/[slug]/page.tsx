...
export default function IpoDetailsPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const detail = await apiUtils.fetchIpoDetails(slugValue);
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-lg">Loading IPO details...</div>
      </div>
    );
  }
  ...
  const extractKeyMetrics = (data: IpoDetailData | null) => {
    if (!data) return {};
    
    const metrics: Record<string, string> = {};
    
    // Extract from ipo_price array
    if (data.ipo_price && Array.isArray(data.ipo_price)) {
      data.ipo_price.forEach(([key, value]) => {
        if (key.toLowerCase().includes('price') || key.toLowerCase().includes('range')) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
            ...
            <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
              ...
              <div className="p-6">
                {activeTab === 'overview' && (
                  <OverviewTab detailData={detailData} />
                )}
                {activeTab === 'financials' && (
                  <FinancialsTab detailData={detailData} />
                )}
                {activeTab === 'timeline' && (
                  <TimelineTab detailData={detailData} />
                )}
                {activeTab === 'documents' && (
                  <DocumentsTab detailData={detailData} />
                )}
              </div>
            </div>
          </div>
