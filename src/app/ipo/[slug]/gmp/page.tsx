...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return generateIpoPageMetadata('gmp', basicData, detailData);
}
...
export default async function GmpPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  ...
  return (
    <main className="container py-10">
      <h1 className="text-3xl font-bold">GMP Price</h1>
